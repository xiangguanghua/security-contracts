...
contract NudgeCampaignFactoryTest is Test {
  ...
  function test_DeploymentValidation() public {
    ...
    factory.deployCampaign(
      holdingPeriod, address(0), address(rewardToken), REWARD_PPQ, campaignAdmin, startTimestamp, withdrawalAddress, 1
    );

    vm.expectRevert(); // Zero holding period
    factory.deployCampaign(
      0, address(targetToken), address(rewardToken), REWARD_PPQ, campaignAdmin, startTimestamp, withdrawalAddress, 1
    );
  ...
  function test_GrantRoleUnauthorized_Reverts() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IAccessControl.AccessControlUnauthorizedAccount.selector, operator, factory.DEFAULT_ADMIN_ROLE()
      )
    );
  ...
  function test_RevokeRoleUnauthorized_Reverts() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IAccessControl.AccessControlUnauthorizedAccount.selector, endUserOne, factory.DEFAULT_ADMIN_ROLE()
      )
    );
  ...
  function test_UpdateFeeSetting_Unauthorized_Reverts() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IAccessControl.AccessControlUnauthorizedAccount.selector, operator, factory.NUDGE_ADMIN_ROLE()
      )
    );
    factory.updateFeeSetting(newFeeBps);
    vm.stopPrank();
