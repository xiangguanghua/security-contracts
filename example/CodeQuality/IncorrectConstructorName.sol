...
Incorrect Constructor Name 攻击 是指由于智能合约中构造函数的命名与合约名称不一致，导致构造函数未被正确识别，从而可能引发安全漏洞或意外行为。在 Solidity 0.4.22 版本之前，构造函数的命名必须与合约名称完全一致。如果命名不一致，编译器会将其视为普通函数，而不是构造函数，这可能导致合约初始化失败或状态未正确设置。
